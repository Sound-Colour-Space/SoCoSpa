...
<a href="vimu_diagArchive.php" target="_blank">Diagram archive</a><br>
<?php
	parse_str($_SERVER['QUERY_STRING']); // not recommended!! 	
	$sql0 = "SELECT gid, Bezeichnung, Beschreibung, n_x, n_y FROM wall WHERE ID = $wall_ID";
	$sql1 = "SELECT id_museobj, x_pos, y_pos,x_span, y_span FROM v_walls_with_museobjs WHERE id_wall = $wall_ID";
	//$n_x = 1;
	//$n_y = 1;
try {
